...
(define-datatype lambda-expr lambda-expr?
  ...
  (appl-expr
   (rator lambda-expr?)
   (rands (genl-list-of lambda-expr?)) ) )
;;   (rands expr-list?) ) )
...
(define occurs-free-helper
  (lambda (x rands)
    (cond ((null? rands) #f
           ((occurs-free? x (car rands)) #t)
           (else (occurs-free-helper x (cdr rands))) ) ) ) )
...
(define occurs-bound-helper
  (lambda (x rands)
    (cond ((null? rands) #f)
          ((occurs-bound? x (car rands)) #t)
          (else (occurs-bound-helper x (cdr rands))))))
